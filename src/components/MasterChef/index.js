...
const MasterChefs = () => {
  ...
  return (
    <div className="masterChef-container">
      ...
      <div className="master-chef-carousel">
        <Slider {...settings}>
          {chefs.map((chef) => (
            <div key={chef.id}>
              <div className="master-chef-card">
                <div className="chef-image">
                  <img src={chef.chefImageUrl} alt={chef.chefName} />
                </div>
                <h3>{chef.chefName}</h3>
                <p className="chef-role">{chef.chefRole}</p>
                <p className="chef-quote">“{chef.chefQuote}”</p>
                <div className="chefs-social-media-card">
                  <a
                    href={chef.chefSocial.facebook}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <FaFacebookF className="chef-social-media-icons" />
                  </a>
                  <a
                    href={chef.chefSocial.instagram}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <FaInstagram className="chef-social-media-icons" />
                  </a>
                  <a
                    href={chef.chefSocial.twitter}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <FaXTwitter className="chef-social-media-icons" />
                  </a>
                </div>
              </div>
            </div>
